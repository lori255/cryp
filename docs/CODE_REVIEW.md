...
| T-21 | P1 | 后台导入覆盖同路径文件前没有停止该路径的 HLS；旧 FFmpeg 可能在密文被截断/替换期间继续读取，产生混合分片并放大后续转码失败。 | 已修复：导入逐文件替换前调用同路径 HLS 停止回调，停止超时则中止该文件。 |
...
| T-25 | P2 | HLS owner 以 `sessionID` 集合去重，同一会话打开两个同路径播放器时，一个播放器关闭会删除唯一 owner 并停止另一播放器仍在使用的 stream。 | 待修复：为播放实例分配独立 lease/token，或至少维护同会话引用计数。 |
| M-06 | P1 | 缩略图旧 generation 任务在空文件路径返回 `skipped` 时，无条件写入失败冷却；若新文件/新任务已排队，会被旧任务错误地抑制 5 分钟。 | 待修复：仅当 generation 仍是当前代时写入 `failed` 状态，并补充替换竞态测试。 |
...
| T-26 | P1 | 删除目录/批量删除目录时只停止目录自身的 HLS；递归删除子视频前没有停止子路径 stream，FFmpeg 可能继续读取被删除/替换的密文并产生失败分片。 | 待修复：按路径前缀收集并等待所有后代 stream。 |
