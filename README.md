...
| 文件元数据 | 派生密钥 + AES-256-GCM | 加密保存时长、编码、分辨率等可扩展属性 |
| 长文件名 | SHA-256 映射 | 处理超出文件系统限制的密文名称 |

Cryp 的加密实现仍应视为应用级安全组件，而不是经过独立密码学审计的通用库。请只从可信来源获取镜像，并为公开访问配置 HTTPS。

媒体元数据和缩略图属于可重建的派生数据。上传和目录导入完成后会通过 Range 解密流生成；历史文件只能通过界面中的“重建索引”显式补齐。登录、浏览目录、请求缩略图和播放视频都不会暗中扫描或回填旧数据。重建索引会重新生成媒体元数据和缩略图，因此大型保险库可能需要较长时间。
